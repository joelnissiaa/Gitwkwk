...
<head>
    ...
    <style>
        ...
        body {
            ...
            margin: 0;
            padding: 0;
            background-color: rgba(168, 202, 236, 0.91);
        }
        
        header {
            background-color:rgba(32, 119, 206, 0.75);
            color: white;
            padding: 10px 20px;
        }
        ...
        display: inline-block;
        padding: 10px 20px;
        background-color:rgb(189, 107, 14); /* Green background */
        color: white;
        font-size: 16px;
        font-weight: bold;
        text-align: center;
...
// Query untuk mendapatkan data siswa dengan pagination
$sql = "SELECT * FROM siswa LIMIT $start, $limit";
$result = mysqli_query($conn, $sql);

// Tombol untuk Create dan Export
echo '<a href="createsiswa.php" class="btn-add-siswa">Tambah Siswa</a> | ';
echo '<a href="export-pdf-siswa.php" class="btn-add-siswa">Export PDF</a><br><br>';

// Tampilkan data dalam tabel
if (mysqli_num_rows($result) > 0) {
    echo "<table border='1'>";
    echo "<tr><th>NIS</th><th>Nama Siswa</th><th>Alamat</th><th>Jenis Kelamin</th><th>Kelas</th><th>Tempat Tanggal Lahir</th><th>Foto</th><th>Action</th></tr>";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if (!empty($row['foto'])) {
            echo "<td><img src='assets/img/team/" . $row['foto'] . "' width='100'></td>";
        } else {
            echo "<td>No photo</td>";
        }
        ...
        // Tombol Action
        echo "<td>
            <form action='detailsiswa.php' method='get' style='display:inline;'>
                <input type='hidden' name='nis' value='" . $row['nis'] . "'>
                <button type='submit' class='btn-update'>Detail</button>
            </form> |
            <form action='updatesiswa.php' method='get' style='display:inline;'>
                <input type='hidden' name='nis' value='" . $row['nis'] . "'>
                <button type='submit' class='btn-update'>Update</button>
            </form> | 
            <form action='deletesiswa.php' method='get' style='display:inline;' onsubmit='return confirm(\"Apakah Anda yakin ingin menghapus data siswa ini?\")'>
                <input type='hidden' name='nis' value='" . $row['nis'] . "'>
...
$total_sql = "SELECT COUNT(*) AS total FROM siswa";
